...
%% 单频压制干扰注入 (CP干扰)
%% ========================================================================
%% 5. 对每个物理帧叠加 0dB 的单频压制干扰 (精准贴合，无外扩)
fprintf('\nStep 5: [电子战攻击] 正在对每个真实帧执行 0dB 单频压制干扰注入 (精准覆盖)...\n');
% 干扰波参数设定
...
for i = 1:num_frames
    % 精确定位当前帧的 PSS 完整物理起始位置 (含 CP)
    real_start_idx = locs_real(i) - pss_len + 1;
    pss_body_start_idx = real_start_idx + N_cp_samples;
    
    % 【核心修改】：刚好卡在 PSS 上，取消提前量，长度严格等于 pss_len
    jam_start_idx = real_start_idx;
    jam_end_idx = real_start_idx + pss_len - 1;
    
    if jam_start_idx > 0 && jam_end_idx <= length(x_raw)
        % 相对时间轴，锚定到 pss_body_start_idx 时相位为 0
...
fprintf('          成功注入所有单频压制干扰 (已实现 100%% 精准贴合，无缝隙、无多余覆盖)！\n');
